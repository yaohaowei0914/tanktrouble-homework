...
Game::Game(string map_file_address)
{
    ...
    game_window = new Window(win_width * config.GRID_SIZE + 1, win_height * config.GRID_SIZE + 1, TITLE);
...
void Game::start()
{
    ...
    first_tank = Tank(1, config.NORMAL, first_tank_pos, rand() % 360, map, bullet_rep, game_window);
    second_tank = Tank(2, config.NORMAL, second_tank_pos, rand() % 360, map, bullet_rep, game_window);
...
void Game::handle_game()
{
    while(true) 
    {
        try 
        {
            ...
            game_window->play_sound_effect(Assets_Dir + config.SLASH + Tank_Explosion + Wav);
                 ...
                 Point& first_tank_pos, Point& second_tank_pos)
{
    ...
    first_tank_pos = Point((first_tank_y + 0.5) * config.GRID_SIZE, (first_tank_x + 0.5) * config.GRID_SIZE);
    second_tank_pos = Point((second_tank_y + 0.5) * config.GRID_SIZE, (second_tank_x + 0.5) * config.GRID_SIZE);
    ...
    for(int i = 0; i < wall_alignment.size(); i++) 
    {
        for(int j = 0; j < wall_alignment[i].size(); j++) 
        {
            switch(wall_alignment[i][j]) 
            {
                ...
                case ONE : 
                {
                    Wall upper_wall( Point(j * config.GRID_SIZE, i * config.GRID_SIZE), Point((j + 1) * config.GRID_SIZE, i * config.GRID_SIZE) );
                ...
                case TWO: 
                {
                    Wall left_wall( Point(j * config.GRID_SIZE, i * config.GRID_SIZE), Point(j * config.GRID_SIZE, (i + 1) * config.GRID_SIZE) );
...
void complete_map(Map* map, const vector <string>& wall_alignment)
{
    ...
    Point down_left(0, map_height * config.GRID_SIZE);
    Point down_right(map_width * config.GRID_SIZE, map_height * config.GRID_SIZE);
    Point top_right(map_width * config.GRID_SIZE, 0);
